...
const fetchReviewers = async () => {
  loading.value = true
  try {
    const result = await getData('/reviewers/')
    reviewers.value = Array.isArray(result) ? result : []
    console.log('获取审核员列表成功:', result)
  } catch (err: any) {
    error.value = err.message || '获取审核员列表失败'
    console.error('获取审核员列表错误:', err)
  } finally {
    loading.value = false
  }
}
    ...
    <div v-else>
      ...
      <div v-else class="table-container">
        <table class="data-table">
          ...
          <tbody>
            <tr v-for="reviewer in reviewers" :key="reviewer.staff_id">
              <td>{{ reviewer.staff_id }}</td>
              <td>{{ reviewer.name }}</td>
              <td>
